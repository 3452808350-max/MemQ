...
print("="*70)
print("MemQ Pro 实用版测试（BM25+ 向量→RRF→质量分，无 Rerank）")
print("="*70)

print("\n🔧 初始化 MemQ Pro 实用版...")
memq = MemQPro(
    cache_dir="/home/kyj/.openclaw/workspace/memq/cache_practical",
    cache_ttl_days=7,
    max_cache_size=1000
)
...
for query, expected in retrieval_tests:
    start = time.time()
    # 无 Rerank
    results = memq.search(query, top_k=5, use_hybrid=True, use_rerank=False, use_quality=True)
...
with open("/home/kyj/.openclaw/workspace/memq/test_report_practical.json", 'w', encoding='utf-8') as f:
    json.dump(report, f, ensure_ascii=False, indent=2)

print(f"\n📄 报告已保存：/home/kyj/.openclaw/workspace/memq/test_report_practical.json")
