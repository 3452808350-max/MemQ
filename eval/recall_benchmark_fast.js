#!/usr/bin/env node
/**
 * Synthetic PerLTQA 召回率基准测试 - 快速版
 * 使用阿里云 Embedding API + 直接 ID 匹配（跳过 LLM 评估）
 */
...
async function runScaleTest(db, scaleName) {
  ...
  const table = await db.createTable(scope, [{
    id: 'init', text: 'init', embedding: memoryEmbeds[0]
  }]);
  ...
  for (let i = 0; i < memories.length; i += batchSize) {
    ...
    const records = batch.map((m, idx) => ({
      id: m.id, text: m.content, embedding: embeds[idx]
    }));
...
async function main() {
  ...
  for (const scale of SCALES) {
    const stats = JSON.parse(fs.readFileSync(path.join(config.dataDir, `stats_${scale}.json`), 'utf-8'));
    const r = allResults[scale];
    console.log(`${scale},${stats.num_memories},${r.recall_at_5},${r.recall_at_10},${r.mrr},${allTimings[scale]}`);
  }
}
