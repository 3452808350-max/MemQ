#!/usr/bin/env python3
"""
Synthetic PerLTQA 召回率基准测试 - Qwen3-Embedding-8B 版
使用 ModelScope 调用 Qwen3-Embedding-8B
"""
...
# ============ 加载模型 ============
print("🚀 下载/加载 Qwen3-Embedding-8B 模型...")
print("⚠️  检测到 CPU 仅支持 AVX1，使用纯 CPU 模式...")

# 设置环境变量禁用优化
os.environ['SENTENCE_TRANSFORMERS_NO_VECTORIZE'] = '1'
os.environ['OMP_NUM_THREADS'] = '4'

model_dir = snapshot_download(MODEL_ID, cache_dir='/home/kyj/.cache/modelscope')
model = SentenceTransformer(model_dir, device='cpu', model_kwargs={'torch_dtype': 'float32'})
print("✅ 模型就绪\n")
...
def run_scale_test(db, scale_name):
    ...
    # 生成 scope
    scope = f"perltqa-qwen3-{scale_name}-{int(time.time())}"
    ...
    # 生成记忆 embedding
    print(f"\n  🚀 生成记忆 embedding (Qwen3-8B)...")
    start = time.time()
    memory_texts = [m['content'] for m in memories]
    memory_embeddings = model.encode(memory_texts, batch_size=32, show_progress_bar=True).tolist()
    ...
    query_embeddings = model.encode(query_texts, batch_size=32, show_progress_bar=False).tolist()
...
def main():
    print("="*70)
    print("🧪 Synthetic PerLTQA 召回率基准测试")
    print("🤖 模型：Qwen3-Embedding-8B")
    print("💻 设备：CPU (ROCm 不稳定)")
    print("="*70)
    ...
    output_path = f"{DATA_DIR}/benchmark_result_qwen3.json"
